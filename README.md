...
Open http://127.0.0.1:3000. The existing `.env` supplies `ERP_DB_SERVER`, `ERP_DB_DATABASE`, `ERP_DB_USER` and `ERP_DB_PASSWORD`. Keep that file private; it is ignored by Git and is never served by the application. `.env.example` documents configuration without credentials.
